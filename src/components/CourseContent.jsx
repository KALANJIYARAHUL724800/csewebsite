...
const CourseContent = () => {
	...
	useEffect(() => {
		searchCourseContent(id)
			.then((res) => {
				const data = res.data.courseId
					? { ...res.data.courseId, ...res.data }
					: res.data;
				setCourseData(data);
				setLoading(false);
			})
	...
	const courseSections = [
		{
			...
			title: "Course Title",
			text: courseData.courseName || courseData.courseTitle,
		},
		{
			icon: <FaLaptopCode size={30} />,
			title: "What You Will Learn",
			text: courseData.whatYouWillLearn,
		},
		{
			icon: <FaUserFriends size={30} />,
			title: "Who Can Join",
			text: courseData.whoCanJoin,
		},
		{
			icon: <FaTools size={30} />,
			title: "Skills You Will Gain",
			text: courseData.skillsYouWillGain,
		},
		{
			icon: <FaListAlt size={30} />,
			title: "Course Topics",
			text: courseData.courseTopics,
		},
		{
			icon: <FaBriefcase size={30} />,
			title: "Career Opportunities",
			text: courseData.careerOpportunities,
		},
	];

	const images = courseData?.logoUrl ? [courseData.logoUrl] : [];
	...
	const handleFormSubmit = async (e) => {
		...
		try {
			...
			const courseData = res.data.courseId
				? { ...res.data.courseId, ...res.data }
				: res.data;
			...
			if (error.response && error.response.status === 400) {
				...
				if (message.toLowerCase().includes("name")) {
					newErrors.name = message
						.split(";")
						.find((m) => m.toLowerCase().includes("name"))
						?.trim();
				}
				if (message.toLowerCase().includes("phone")) {
					newErrors.phone = message
						.split(";")
						.find((m) => m.toLowerCase().includes("phone"))
						?.trim();
				}
				...
				{courseSections.map((section, idx) => (
					<div key={idx} className="col-12 col-sm-6 col-lg-4">
						<div
							...
							<div className="mb-3" style={{ color: "#007bff" }}>
								{section.icon}
							</div>
							<h5 className="mb-2 fw-bold heading">{section.title}</h5>
							<p className="mb-0 para">{section.text || "N/A"}</p>
						</div>
					</div>
				))}
			</div>
			...
			{showSuccess && (
				<div
					...
					<div
						...
						<p className="para">
							Our staff will contact you shortly regarding the fees.
						</p>
					</div>
				</div>
			)}
